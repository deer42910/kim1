...
![image-20240910131451605](D:\a_briup_learn\redis\02-实战篇\讲义\Redis实战篇.assets\image-20240910131451605.png)
...
**Session**(会话)是指在用户与服务器之间的交互过程中，服务器为用户分配的一段时间段。这段时间内，服务器可以保存用户的状态与信息。

- 登录状态：保存用户的登录信息，确保用户在不同页面间的访问无需重新登录。
- 购物车：记录用户选择的商品，直到用户完成购物或离开网站。

session通常由一个唯一的标识符（如cookie)管理，服务器使用这个标识符来关联用户会话数据。
...
缓存雪崩是指在同一时段大量的缓存key同时失效(过期了)或者Redis服务宕机，导致大量请求到达数据库，带来巨大压力。

解决方案：

* 给不同的Key的TTL添加随机值  –解决同一时间失效问题
* 利用Redis集群提高服务的可用性  –高级课程中，解决宕机
...
缓存击穿问题也叫热点Key问题，就是一个**被高并发访问**（同一时间都来访问一个key）并且**缓存重建业务较复杂**(所以重建缓存的时间比较长)的key突然失效了，无数的请求访问会在瞬间给数据库带来巨大的冲击。
...
* 获取锁：
  ...
  * 非阻塞：尝试一次，成功返回true，失败返回false
  ...
  #添加锁，利用setnx的互斥特征
  SETNX lock thread1
  #添加一个过期时间，避免服务宕机引发的死锁
  EXPIRE lock 10  #10秒钟
  ```

  注意要是在添加锁与添加过期时间之间服务宕机

  ```
  SET lock thread1 EX 10 NX   
  #一起设置保证原子性 EX过期时间 NX（互斥）已有不能创建
  ```
...
![551dd57f808b90870bde8044c1ef2a9](D:\a_briup_learn\redis\02-实战篇\讲义\Redis实战篇.assets\551dd57f808b90870bde8044c1ef2a9.jpg)
...
写好脚本以后，需要用Redis命令来调用脚本(script)，调用脚本的常见命令如下：
...
local key = KEYS[1]
--当前线程标识  UUID
local threadId = "uihdeiuhuhef-33"
