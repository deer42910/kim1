...
**并发（concurrent）是同一时间应对（dealing with）多件事情的能力**  （能做）
...
2. Callalbe接口支持返回执行结果，需要调用FutureTask.get()得到，此方法**会阻塞主进程的继续往下执行**，如果不调用不会阻塞。
...
- 使用退出标志，使线程正常退出，也就是当run方法完成后线程终止（这个也不算中断，是正常运行结束）
- 使用stop方法强行终止（不推荐，方法已作废）
- 使用interrupt方法中断线程（一种是阻塞状态+interrupt会抛出异常，判断中断，然后break）
- ??抛出异常  run()方法是重写的，父类没有抛出受检异常所以不能抛出，未受检异常可以抛出？
...
**原子性**可以通俗理解为“不可分割性”。在计算机事务中，它确保要么所有操作都成功，要么所有操作都失败，不会出现中间状态。加锁保护原子性是通过锁定资源来防止其他操作干扰，从而确保操作的完整性。
...
public class TicketDemo {
    ...
    public synchronized void getTicket() {
        synchronized (this) {
            ...
            // 非原子性操作 原子性：事务是一个不可分割的工作单位，事务中的操作要么都发生要么都不发生
            ticketNum--;
        }
    }
...
- Monitor实现的锁属于重量级锁，里面涉及到了用户态和内核态的切换、进程的上下文切换，成本较高，性能比较低。
  ...
  > 用户态：（需要完成的功能可以在自己进程内部处理，也就是线程）  
  >
  > 内核态：cpu层面的东西  jvm是系统级别属于内核态
  >
  > Linux:一种操纵系统的内核  ubantu\centos：基于内核的发行版
  >
  
- 在JDK 1.6引入了两种新型锁机制：偏向锁和轻量级锁，它们的引入是为了解决在**没有多线程竞争或基本没有竞争的场景下**因使用传统锁机制带来的性能开销问题。
...
我们需要重点分析MarkWord对象头  （他和moniter怎么关联）
...
- 后三位是001表示无锁  
  ...
  > 某一段代码/方法使用了synchronized修饰，有同步代码块声明，但是没有实际调用的线程。
  >
  > 若有第一条线程来访问这段代码，首先就会偏向锁的升级

- 后三位是101表示偏向锁

  > 有一段代码使用了synchronized修饰，并且也有线程来访问，但每次来的都是同一条线程
  >
  > 锁底层有个记录偏向的线程id,若下次访问的线程与记录的偏向线程id相同时，直接进入执行，不会触发申请互斥操作。
  >
  > 当出现了第二条线程，同样也访问这段代码，偏向锁会膨胀为轻量级锁

- 后两位是00表示轻量级锁

  > 有一段代码使用synchronized修饰，并且多条线程对这段代码就行访问，但没有实际产生竞争或少量竞争，任何一条线程访问的时候都会发现锁是可以的
  >
  > 当线程数据继续上升，或产生了实际竞争的时候，轻量级锁会膨胀为重量级锁

  自旋操作

    发现锁不可用时，并不直接将自己挂起，而是通过循环的方式，在一段时间内反复判断锁得状态，侥幸锁会在短时间内被当前持有者释放掉。

     jdk1.6之前，自旋锁是有固定的执行上限。

     之后，自旋锁的次数上线变成了不固定值，自旋锁进化为自适应自旋锁，根据线程执行的情况，动态判断自旋上线。

- 后两位是10表示重量级锁

  > synchronized的抢锁原理，申请量，若申请不到，则线程将自己挂起
...
> 内存：RAM随机存取存储器（random access memory)  cpu直接交换数据的内部存储器，也叫主存(内存)。随读随写，速度很快，作为操作系统或其他正在运行中的程序的临时数据存储媒介。当电源关闭时RAM不能保存数据。若要保存数据，就必须把他们写入一个长期的存储设备中(例如硬盘)。我们通常购买或升级的内存条就是用作电脑的内存，内存条（SIMM）就是将RAM集成块集中在一起的一小块电路板，它插在计算机中的内存插槽上，以减少RAM集成块占用的空间。
>
> ROM只读存储器 信息只读出，不能写入，即使计算机掉电，数据不会消失，主要存放计算机的基本程序和数据（底层的不能改变的）。
>
> cache高速缓存区 cpu与内存之间，读写速度比内存更快。当CPU向内存中写入或读出数据时，这个数据也被存储进高速缓冲存储器中。当CPU再次需要这些数据时，CPU就从高速缓冲存储器读取数据，而不是访问较慢的内存，当然，如需要的数据在Cache中没有，CPU会再去读取内存中的数据。
...
CAS 底层依赖于一个 Unsafe 类来直接调用**操作系统底层的** CAS 指令

![image-20230504182737931](多线程相关面试题.assets/image-20230504182737931.png)

都是**native修饰的方法**，由系统提供的接口执行，并非java代码实现，一般的思路也都是自旋锁实现

> native修饰的方法都是系统本地的 不是java语言而是c/c++
