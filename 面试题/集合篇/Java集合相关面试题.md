...
![image-20240826213117642](D:\a_briup_learn\面试题\集合篇\Java集合相关面试题.assets\image-20240826213117642.png)
...
>将collection对象转换成数组，然后将**数组的地址**的赋给elementData
...
- 操作数据效率
  - ArrayList按照下标**查询**的时间复杂度O(1)【内存是连续的，根据寻址公式】， LinkedList不支持下标查询
...
> hash表（散列表）即数组+（链表|红黑树）

1. 当我们往HashMap中put元素时，利用key的hashCode重新hash计算出当前**对象的元素在数组中的下标** 
...
当我们往数组中添加的元素超过 数组容量*加载因子(0.75f) 时，进行扩容
...
> 假设哈希表的容量（`oldCap`）是8，即二进制为`1000`。一个元素的哈希值为`6`，二进制为`0110`。计算`e.hash() & oldCap`即`0110 & 1000`，结果是`0000`（0）。e.hash()是随机的
>
...
首先获取key的hashCode值，然后右移16位 异或运算 原来的hashCode值，**主要作用就是使原来的hash值更加均匀，减少hash冲突（扰动算法）**
...
> (16-1)&97=1
>
> **关于位与运算(&)可以代替取模运算(%)**它们都能用于确定一个值在一定范围内的位置。
>
> 限制条件：‘n’是2的幂时，可以替换
>
> &逐位进行比较，并在两个数值的相应位都为1时结果位为1，否则为0
>
> 例如：8二进制`1000` 8-1:`0111`  `x&7``x%9`
>
> 为什么位与运算更高效
>
> - 位与运算是硬件支持的基本操作，通常比取模运算更快。
>
> - 对于 `n` 是 2 的幂的情况，`x % n` 通过 `x & (n - 1)` 计算更加高效，因为它仅涉及位操作，而不需要除法和取余操作。

![image-20230428212729580](Java集合相关面试题.assets/image-20230428212729580.png)

(n-1)&hash : 得到数组中的索引，代替**取模**，性能更好，数组长度必须是2的n次幂
